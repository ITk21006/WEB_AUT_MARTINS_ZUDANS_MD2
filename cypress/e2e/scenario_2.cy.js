describe('CURA Healthcare Service', () => {
  ...
  it('Appointment history empty', () => {
    ...
    // Step v: Click the Menu/Stack/Burger icon
    cy.get('#menu-toggle').click();

    // Step vi: Validate that the sidebar is active
    cy.get('#sidebar-wrapper').should('have.class', 'active');

    // Step vii: Click "History"
    cy.get('a').contains('History').click();

    // Step viii: Validate that "No appointment" is visible
    cy.get('.col-sm-12 > p').should('have.text', 'No appointment.');
  });
});
